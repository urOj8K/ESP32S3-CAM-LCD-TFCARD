...
uint8_t SCCB_Probe(void)
{
    ...
    //     ESP_LOGE(TAG, "SCCB_Probe 00000000000 %d %x",i,ret);
    //    if( ret == ESP_OK) {
    ...
    for (size_t i = 0; i < CAMERA_MODEL_MAX; i++) {
       ...
       i2c_cmd_link_delete(cmd);
       if( ret == ESP_OK) {
             ESP_LOGE(TAG, "Found I2C Device at 0x%02X", slave_addr);
           return slave_addr;
        }
   }
    return 0;
...
uint8_t SCCB_Write(uint8_t slv_addr, uint8_t reg, uint8_t data)
{
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();
    i2c_master_start(cmd);
    ret = i2c_master_write_byte(cmd, ( slv_addr << 1 ) | WRITE_BIT, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, reg, ACK_CHECK_EN);
    i2c_master_write_byte(cmd, data, ACK_CHECK_EN);
    i2c_master_stop(cmd);
    ret = i2c_master_cmd_begin(SCCB_I2C_PORT, cmd, 1000);
    i2c_cmd_link_delete(cmd);
    if(ret != ESP_OK) {
