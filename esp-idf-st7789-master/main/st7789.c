...
void lcdInit(TFT_t * dev, int width, int height, int offsetx, int offsety)
{
	...
	////end of st7735s frame rate

	//	spi_master_write_command(dev, 0xB4);  //dot inversion
	//	spi_master_write_data_byte(dev, 0x03);

	//---------------------------------ST7789S Power setting--------------------------------------//

	spi_master_write_command(dev, 0xBB); //Set VRH1[4:0] & VC[2:0] for VCI1 & GVDD
	spi_master_write_data_byte(dev, 0x30);  //vcom


	spi_master_write_command(dev, 0xC3); //Set BT[2:0] for AVDD & VCL & VGH & VGL
	spi_master_write_data_byte(dev, 0x17);

	spi_master_write_command(dev, 0xC4); //Set BT[2:0] for AVDD & VCL & VGH & VGL
	spi_master_write_data_byte(dev, 0x18);


	spi_master_write_command(dev, 0xC6); //Set BT[2:0] for AVDD & VCL & VGH & VGL
	spi_master_write_data_byte(dev, 0x0F);


	spi_master_write_command(dev, 0xD0); //Set BT[2:0] for AVDD & VCL & VGH & VGL
	spi_master_write_data_byte(dev, 0xA4);
	spi_master_write_data_byte(dev, 0xA2);


	spi_master_write_command(dev, 0x36);        //mx,my,rgb mode
	//spi_master_write_data_byte(dev, 0xC8);     //photo, normal
	//spi_master_write_data_byte(dev, 0xe8);       //scene, top to bottom
	//spi_master_write_data_byte(dev, 0x68);       //scene, top to bottom,left to right 
	spi_master_write_data_byte(dev, 0xa0);       //scene, top to bottom
	...
	spi_master_write_data_byte(dev, 0x47);
	...
	spi_master_write_data_byte(dev, 0x3a);
	spi_master_write_data_byte(dev, 0x17);
	spi_master_write_data_byte(dev, 0x16);
	spi_master_write_data_byte(dev, 0x21);
	spi_master_write_data_byte(dev, 0x22);

	spi_master_write_command(dev, 0x3a);   //65k mode
	spi_master_write_data_byte(dev, 0x55);  //RGB 565  16bit

	//  ste window  //
	spi_master_write_command(dev, 0x2A); //Frame rate control
	spi_master_write_data_byte(dev, 0x00);
	spi_master_write_data_byte(dev, 0x00);
	spi_master_write_data_byte(dev, 0x00);
	spi_master_write_data_byte(dev, 0xEF);

	spi_master_write_command(dev, 0x2B); //Display function control
	spi_master_write_data_byte(dev, 0x00);
	spi_master_write_data_byte(dev, 0x00);
...
void lcdShowPiture(TFT_t * dev, uint8_t *buf){
	static uint8_t Byte[1024];
	spi_master_write_command(dev, 0x2A);	// set column(x) address
	...
	gpio_set_level( dev->_dc, SPI_Data_Mode );
	for(int i=0;i<240;i++)	//work in 320x240 mode
	{
		memcpy(Byte,buf+i*640,640);
		spi_master_write_byte( dev->_SPIHandle, Byte, 640);
	}
}
